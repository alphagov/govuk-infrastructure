...
In DATE we had an IT Health Check conducted which flagged some critical vulnerabilities in the fundamental architecture
...
We have decided to move to using OCI images instead of a Git repository for the purpose of storing the manifests that
...
we choose for YAML generation. The rendered manifest files will then be stored in an OCI image, tagged, and pushed to
an OCI image repository. In this way, they will act exactly like the containers we use for application source code.

ArgoCD will [fetch from the OCI image repository](https://argo-cd.readthedocs.io/en/stable/user-guide/oci/) and
synchronise the manifests in the same way it currently does with a Git repository.

We think that using OCI images has two distinct advantages:

1. We make the way the manifests are rendered independent of how they are deployed
2. We have full versioned artifacts representing the state of the infrastructure needed by an application, which can be
   rolled forwards and back just like application code.
...
5. Render manifests
6. Store in OCI image with Git SHA tag, push to registry
7. Update ArgoCD application to reference new OCI image
...
env_store["Store OCI image with Git SHA tag, push to registry"] -->
env_wait["Update ArgoCD application to reference new OCI image"] -->
...
## Abstraction with Crossplane

WRITE ABOUT CROSSPLANE
...
In each environment we expect to run at least one new node in Kubernetes for Concourse, plus its database. In the nww
