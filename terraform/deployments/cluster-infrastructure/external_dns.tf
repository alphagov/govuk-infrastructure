...
locals {
  ...
  external_dns_zone_name            = trimsuffix("${var.external_dns_subdomain}.${data.terraform_remote_state.root_dns.outputs.external_root_zone_name}", ".")
}
...
resource "aws_route53_record" "cluster_public_ns_parent" {
  zone_id = data.terraform_remote_state.root_dns.outputs.external_root_zone_id
