...
module "ckan_iam_role" {
  ...
  version                       = "~> 4.0"
  create_role                   = true
  role_name                     = "${var.ckan_service_account_name}-${var.cluster_name}"
...
resource "aws_iam_policy" "ckan" {
  name        = "EKS-CKAN-${var.cluster_name}"
  description = "EKS ${var.ckan_service_account_name} policy for cluster ${module.eks.cluster_id}"
