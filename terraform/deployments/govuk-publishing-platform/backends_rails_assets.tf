resource "aws_s3_bucket" "backends_rails_assets" {
  bucket = "govuk-${var.govuk_environment}-${local.workspace}-backends-rails-assets"
  ...
  tags = {
    name            = "govuk-${var.govuk_environment}-${local.workspace}-backends-rails-assets"
...
data "aws_iam_policy_document" "backends_cloudfront_access_s3_policy" {
  statement {
    ...
    resources = ["${aws_s3_bucket.backends_rails_assets.arn}/*"]
