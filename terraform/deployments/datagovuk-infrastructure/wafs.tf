# AWS WAF for Find and CKAN applications rate limiting
# This is to implement and add WAF to nginx ingress to address 503 errors

# ===========================================================
# Data sources to find ALBs using Kubernetes tags
# ===========================================================
...
data "aws_lb" "ckan" {
  tags = {
    "elbv2.k8s.aws/cluster" = "govuk"
    "ingress.k8s.aws/stack" = "datagovuk/ckan"
  }
}
...
# ===========================================================
# WAF Association and CloudWatch Logging for Find
# ===========================================================
...
# Main WAF Web ACL for CKAN application
resource "aws_wafv2_web_acl" "ckan" {
  name  = "ckan-${var.govuk_environment}-rate-limiting"
  scope = "REGIONAL"
  default_action {
    allow {}
  }
  ...
  rule {
    ...
    visibility_config {
      cloudwatch_metrics_enabled = true
      metric_name                = "ckan-rate-limit-warning"
      sampled_requests_enabled   = true
    }
  }
  # Rule 2: Rate limit blocking
  ...
  rule {
    ...
    action {
      block {
        custom_response {
          response_code            = 429
          custom_response_body_key = "ckan-rate-limit-429"
          # This is to inform clients to retry after 30 seconds
          response_header {
            name  = "Retry-After"
            value = "30"
    ...
    visibility_config {
      cloudwatch_metrics_enabled = true
      metric_name                = "ckan-rate-limit-block"
      sampled_requests_enabled   = true
    }
  }
  # Custom 429 "Too Many Requests" response page
...
<html lang="en">
  ...
  <body>
    <div class="container">
      <div class="logo">GOV.UK</div>
      <h1>Sorry, there have been too many requests</h1>
      <p>You have made too many requests to CKAN open data in a short period.</p>
      <p>Please wait a few minutes and try again.</p>
      <p>If you need to access large amounts of data, please consider using our <a href="https://www.data.gov.uk/api">API documentation</a> for bulk access options.</p>
    </div>
  </body>
</html>
HTML
  ...
  visibility_config {
    cloudwatch_metrics_enabled = true
    metric_name                = "ckan-web-acl"
    sampled_requests_enabled   = true
  }
  tags = {
    Name        = "ckan-${var.govuk_environment}-waf"
    Application = "CKAN"
    Purpose     = "RateLimiting"
  }
}
...
# ===========================================================
# WAF Association and CloudWatch Logging for CKAN
# ===========================================================

# Associate WAF with CKAN ALB and this attaches the WAF rules to the ALB
resource "aws_wafv2_web_acl_association" "ckan_alb" {
  resource_arn = data.aws_lb.ckan.arn
  web_acl_arn  = aws_wafv2_web_acl.ckan.arn
}

# CloudWatch Log Group for WAF logs
# Stores logs of blocked and counted requests
resource "aws_cloudwatch_log_group" "ckan_waf" {
  name              = "aws-waf-logs-ckan-${var.govuk_environment}"
  retention_in_days = var.waf_log_retention_days
  tags = {
    Name        = "ckan-waf-logs"
    Application = "CKAN"
...
# WAF Logging Config which sends WAF events to CloudWatch Logs
resource "aws_wafv2_web_acl_logging_configuration" "ckan_waf" {
  resource_arn            = aws_wafv2_web_acl.ckan.arn
  log_destination_configs = [aws_cloudwatch_log_group.ckan_waf.arn]
  # Only log blocked and counted requests (not all allowed requests)
  logging_filter {
    default_behavior = "DROP"
    # Keep COUNT actions (warning threshold hits)
