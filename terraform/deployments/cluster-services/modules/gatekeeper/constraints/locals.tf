locals {
  ...
  block_default_namespace_yaml           = yamldecode(file("${local.constraint_path}/block_default_namespace.yaml"))
  ...
  constraint_map = {
    block_default_namespace = merge(local.block_default_namespace_yaml, { "spec" : merge(local.block_default_namespace_yaml.spec, { "enforcementAction" : var.dryrun_map.block_default_namespace ? "dryrun" : "deny" }) })
