locals {
  neptune_subnet_ids = compact([for name, id in data.tfe_outputs.vpc.nonsensitive_values.private_subnet_ids : startswith(name, "rds_") ? id : ""])
...
resource "aws_route53_record" "reader_cname" {
  for_each = {
    for k, v in var.neptune_dbs : k => v if v.internal_cname_domains_enabled
  }
