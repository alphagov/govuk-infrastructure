...
output "reader_cname" {
  description = "neptune cluster reader endpoint cname domain"
  value       = { for k, v in aws_route53_record.reader_cname : k => v.fqdn }
}

output "writer_cname" {
  description = "neptune cluster writer endpoint cname domain"
  value       = { for k, v in aws_route53_record.writer_cname : k => v.fqdn }
}

output "neptune_policy_arn" {
  description = "neptune cluster data policy"
  value       = { for k, v in aws_iam_policy.neptune : k => v.arn }
}
