...
| <a name="input_log_group_name_overrides"></a> [log\_group\_name\_overrides](#input\_log\_group\_name\_overrides) | All options from here down are temporary to enable importing the existing ElasticSearch opensearch cluster | <pre>object({<br/>    index_slow_logs  = string<br/>    search_slow_logs = string<br/>    error_logs       = string<br/>  })</pre> | `null` | no |
